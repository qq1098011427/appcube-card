    ...
    <div class="content">
      ...
      <div id="content-container">
        <div
          :class="{
            'button-prev': true,
            'button-disabled': this.render.currentPage === 1
          }"
          v-show="1 === sindex"
...
export default {
  ...
  async mounted() {
    ...
    console.log('xmmp版本', window.xm.version)
    // 监听更新
    ...
    const render = async (needCache = false) => {
      if (needCache) {
        ...
        console.log("%c [appcube 环境执行取缓存操作]", "color: green;");
        if (list["shenzhen-ioc-sublist"]) {
          this.list = JSON.parse(list["shenzhen-ioc-sublist"]);
          console.log("缓存的shenzhen-ioc-sublist: ", this.list);
          // 数组长度必须达到 cube渲染方法 入参PageSize设置的值
      ...
      let subList = allSubList.filter(
        s =>
          ...
          s.config.componentSymbol.indexOf("hw_") > -1 // hw_
      );
      subList = subList.filter(
        x => x.bizId.indexOf('t0000000000gujujslgo5_Dzt') > -1 //
      );
      let hasRule =  await this.getUserSortingRules(userId).then(res => res.data);
      console.log(hasRule, '规则');
      // 如果订阅卡片大于1张，显示已订阅的卡片，否则显示所有卡片
      if (subList.length >= 1) {
        console.log(subList, '====s====');
        let c1 = await this.getCurrentCardList(userId).then(res => res.data);
        if (hasRule) { // 存在规则
          this.subList = subList.map(s => {
            if (c1.find(c => c.cardId === s.appId)) {
              return s
            }
          })
        } else {
        ...
        console.log("已经订阅且筛选的卡片: ", this.subList);
      } else {
        this.subList = allSubList;
        console.log("订阅卡片小于1张显示全部卡片: ", this.subList);
      }
  ...
  methods: {
    ...
    mutationObDomNode() {
      ...
      let observer = new MutationObserver((mutations) => {
        const list = this.subList.length ? this.subList : this.list
        console.log(mutations, 'mob宽高变化', list)
        list.length && this.hwNewRenderCard(list);
      })
    ...
    // 判断是否有排序规则
    async getUserSortingRules(userId) {
      return window.xm.request({
        url: "/smallapp/v1/subscribeManager/getUserSortingRules.json",
        isRelative: true,
        method: "GET",
        data: { userId },
        withCredentials: true
    ...
    hwNewRenderCard(subList) {
      ...
      if (document.getElementById("subscriptionCardRenderFrame")) {
        document
          ...
          .parentNode.removeChild(document.getElementById("subscriptionCardRenderFrame"));
      }
      //
      console.log('渲染', subList);
      try {
        const render = window.AppCubeRender.initSubscriptionCardRender({
          domain: "https://172.26.98.170:32744", // dzt
          elementId: "content-container",
          tenant: "0000000000gujuJSLgo5", // 深圳（租户）
          pageSize: 4,
          gutter: 0,
          containerWidth: document.querySelector("#content-container").offsetWidth,
          cardHeight: 265,
          bgColor: "#FFFFFF",
          view: this.sindex ? "inline" : "grid",
          cardList: subList.filter(x => x).map(x => x.bizId),
          ak: "09692b7d20c145aaa391aa3a5d136d63", // client_id
          sk: "6e85a967c7763d01b127a6694b784c24994c295c0f09585d" // client_secret
        });
        this.render = render;
        this.render.render();
